...
if __name__ == '__main__':
    ...
    for trained_model in os.listdir(TRAINED_MODELS_PATH):
        ...
        for column in forecast_df.columns:
            ...
            col_id = Filtered_by_date_df['id'][0]
            value = forecast_df[column][0]
            idList.append(col_id)
            valueList.append(value)

        ResultsDf = pd.DataFrame({'id': idList, 'sales': valueList}).sort_values('id')
        store_family = model_detail_list[1].replace(r'\w', '_').replace(',', '_').replace(r'/', '_')
        file_name = f'model_results_{model_detail_list[0]}_{store_family}'
        file_path = Path(RESULTS) / file_name
        print(f'writing results to: {file_path}')
        ResultsDf.to_csv(file_path, sep=',', index=False, header=False)
